...
class RewriteSlot : IComponent(), ItemSlot {
    override var background = ref(nrl("textures/ui/top_inventory_itemslot/background.png"))
    override var hovered_background = ref(nrl("textures/ui/top_inventory_itemslot/hovered_background.png"))
    override val slots = ArrayList<Slot>()
    override var lineSize: Attribute<Int> = ref(9)
    val indexList = ref(ArrayList<Int>())

    override fun onOpen(handler: AbstractContainerMenu?) {
        super.onOpen(handler)
        handler?.slots?.find { it.index in indexList.v }?.let {
            slots.add(it)
        }
    }
    override fun render(
